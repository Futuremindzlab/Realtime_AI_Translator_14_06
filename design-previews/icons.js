/* Injects an SVG sprite so every preview can use <svg class="i"><use href="#i-mic"/></svg>.
   Paths are the Lucide icon set (same family the app already uses via lucide-react-native). */
document.addEventListener('DOMContentLoaded', () => {
  ...
  svg.innerHTML = Object.entries(icons)
    .map(
      ([name, path]) =>
        `<symbol id="i-${name}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">${path}</symbol>`,
    )
    .join('');
